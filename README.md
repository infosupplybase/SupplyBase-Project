...
|---|---|---|
| [`frontend/`](frontend) | The public website and client dashboard | Next.js (App Router) |
| [`admin/`](admin) | The staff back-office — enquiries, bookings, projects, payments | Next.js (App Router) |
...
`frontend` and `admin` each read their own `.env.local` — copy `.env.local.example` to `.env.local` in `frontend/` first; `admin/.env.local` only needs `NEXT_PUBLIC_API_URL`. Neither app can do anything useful without the backend running.

## 2. Deploy it

For the production setup, follow [`DEPLOYMENT.md`](DEPLOYMENT.md). It covers deploying `frontend/` and `admin/` as two separate Vercel projects, and `backend/` to Render or Railway, including every environment variable each platform needs.
...
| Colours, fonts, spacing | `frontend/src/styles/variables.css` |
...
| `/services/:slug`, `/booking/:slug` | Book a site visit for one service |
...
| `/dashboard` | Client account page — only opens once signed in; sends `ADMIN` accounts to the admin app instead |
...
In `frontend/`, copy `.env.local.example` to `.env.local`. For local work the defaults are already right:
...
In `admin/`, `.env.local` only needs:

```
NEXT_PUBLIC_API_URL=http://localhost:8080
```

Everything prefixed `NEXT_PUBLIC_` is compiled into the JavaScript a browser
...
3. **APIs & Services → Credentials → Create credentials → OAuth client ID.**
   ...
   - **Authorised JavaScript origins:** `http://localhost:3000` for local work,
...
tokens, so the rest of the site never has to care which route they took. A
`CUSTOMER` lands on `/dashboard`; an `ADMIN` is bounced from `/dashboard`
straight to the admin app.
...
There is **no self-service password reset yet.** The "Forgot password?" link
honestly says to call you instead of pretending an email was sent. Building it
needs a reset-token table and an email template — ask when you want it.
...
frontend/
  ...
  src/
    app/                  routes — Next.js App Router, one folder per path
      (site)/              every page with the navbar/footer chrome
      login/, register/    outside that chrome
    pageComponents/        components shared by more than one route file
                           (ServiceBooking, Login, Legal)
    ...
    components/
      layout/              Navbar, ServiceMegaMenu, MobileMenu, Footer,
                           FloatingActions, ScrollToTop, ProtectedRoute, NavLink
    ...
    styles/                variables.css (tokens), then everything else
...
admin/
  ...
  src/
    app/
      login/                standalone staff sign-in, no sidebar
      (protected)/           guarded by AdminRoute logic baked into its layout —
                             Overview, Enquiries, Bookings, Projects (+ detail), Payments
    components/
      admin/                StatusBadge, Pagination, Drawer, NavLink
      ui/Icon.jsx
...
- [ ] Point `NEXT_PUBLIC_API_URL` (both apps) and `NEXT_PUBLIC_ADMIN_URL` (frontend) at your deployed addresses, not localhost
- [ ] Have a professional review `frontend/src/pageComponents/Legal.jsx` — the privacy policy and terms are starting drafts, not legal advice
