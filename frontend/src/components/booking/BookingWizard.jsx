 ...
 * Property images.
 *
 * These files must be inside:
 *
 * frontend/public/assets/waterproofing/hero/
 */
const propertyImages = {
  '1 BHK': '/assets/waterproofing/hero/one.png',
  '2 BHK': '/assets/waterproofing/hero/two.png',
  '3 BHK': '/assets/waterproofing/hero/three.png',
  '4+ BHK': '/assets/waterproofing/hero/four.png',
...
export default function BookingWizard({
  ...
  console.log("🔥 BOOKING WIZARD IS RUNNING");
  const { user } = useAuth();
  ...
  const services = useMemo(
    () => servicesForLane(lane),
    [lane]
  );

  const selected = services.find(
    (s) => s.slug === form.serviceSlug
  );

  const workOptions =
    serviceWorkOptions[form.serviceSlug] || [];

  const showArea =
    form.serviceSlug &&
    areaRelevant.has(form.serviceSlug);
  ...
  const validateStep = (index) => {
    ...
    if (index === 0 && !form.serviceSlug) {
      next.serviceSlug = 'Please choose a service';
    }

    if (index === 1) {
      if (
        form.serviceSlug === 'waterproofing' &&
        form.waterproofingServices.length === 0
      ) {
        next.waterproofingServices =
          'Please choose at least one waterproofing service';
      } else if (
        form.serviceSlug !== 'waterproofing' &&
        !form.propertyType
      ) {
    ...
    if (index === 2) {
      ...
      if (!form.location.trim()) {
        next.location =
          'Please tell us the area or city';
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await api.createBooking({
        ...
        workOption:
          form.serviceSlug === 'waterproofing'
            ? form.waterproofingServices.join(', ')
            : form.workOption || null,

        workDetail: form.workDetail || null,

        materialSupplier:
          form.materialSupplier || null,
  ...
  if (receipt) {
    return (
      ...
      >
        ...
        <h2>
          YOUR SITE VISIT HAS BEEN REQUESTED
        </h2>
        ...
        {receipt.sendAttachments && (
          ...
          >
            ...
            <span>
              Send your photos and floor plan to{' '}
              <strong>
  ...
  return (
    <div className="booking">
      ...
      {/* Lane switch */}
      <div
        className="booking-lanes"
        role="tablist"
        aria-label="Booking type"
      >
        {Object.entries(bookingLanes).map(
          ([key, value]) => (
            <button
              key={key}
              type="button"
        ...
        {step === 1 && (
          ...
          >
            ...
            'waterproofing' ? (
              <>
                <legend>
                  What waterproofing service do you need?
                </legend>
                ...
                <div className="choice-grid">
                  {waterproofingOptions.map(
                    (option) => {
                      const selected =
                        form.waterproofingServices.includes(
                          option
                        );
                        ...
                        <button
                          ...
                          onClick={() => {
                            setForm((f) => ({
                              ...
                              waterproofingServices:
                                selected
                                  ? f.waterproofingServices.filter(
                                      (item) =>
                                        item !==
                                        option
                                    )
                                  : [
                                      ...f.waterproofingServices,
                                      option,
                                    ],
                            }));
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          boxShadow:
                            isSelected
                              ? '0 0 0 2px rgba(201,162,39,0.25), 0 8px 24px rgba(0,0,0,0.12)'
        ...
        {step === 2 && (
          ...
          >
            ...
            {form.hasAttachments && (
              ...
              >
                We will ask for them on
                WhatsApp right after you
                book.
              </p>
            )}
          </fieldset>
        ...
        {step === 3 && (
          ...
          >
            ...
            >
              ...
              fixed appointment — we will
              confirm the exact time with you.
              Leave it blank and we will
              suggest a time.
            </p>
