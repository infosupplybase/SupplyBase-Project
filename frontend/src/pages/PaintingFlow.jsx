...
export default function PaintingFlow({
  ...
  const resolved = useMemo(() => {
    ...
    return configSteps
      ...
      .map((step, i) => {
        if (step.type === 'brand') {
          const opt = optionsFor('paint_brand').find(
            (o) => o.value === answers.paint_brand
          );
        ...
        if (step.type === 'option') {
          const opt = optionsFor(step.questionKey).find(
            (o) => o.value === answers[step.questionKey]
          );
        ...
        if (step.type === 'product') {
          ...
          const opt = [...tiers.values()]
            .flat()
            .find((o) => o.value === answers[step.questionKey]);
        ...
        if (step.type === 'colour') {
          ...
          const opt = [...tabs.values()]
            .flat()
            .find((o) => o.value === answers[step.questionKey]);

          return {
            stepIndex: i + 1,
            label: step.title.replace('Choose Your ', ''),
            value: opt?.label,
            priceRupees: null,
          };
        }
        ...
        if (step.type === 'addon') {
          ...
          const opts = optionsFor(step.questionKey).filter((o) =>
  ...
  const itemsTotalPaise = useMemo(
    () =>
      resolved.reduce(
        (sum, r) =>
          sum + Math.round((r.priceRupees || 0) * 100),
        0
      ),
    [resolved]
  );
  ...
  const validateStep = (step) => {
    if (step.type === 'option') {
      ...
      const required = question.length > 0;

      if (required && !answers[step.questionKey]) {
        setErrors({
          [step.questionKey]: 'Please choose an option',
        });
    ...
    if (step.type === 'brand' && !answers.paint_brand) {
      setErrors({
        paint_brand: 'Please choose a brand',
      });
    ...
    if (step.type === 'addon' && step.required) {
      ...
      if (chosen.length === 0) {
        setErrors({
          [step.questionKey]: 'Please choose at least one',
        });
      ...
      configSteps.forEach((step) => {
        ...
        if (step.type === 'brand') {
          if (answers.paint_brand) {
            ...
            flat.push({
              key: 'paint_brand',
              value: answers.paint_brand,
              label: opt?.label || answers.paint_brand,
            });
          }
        ...
        if (step.type === 'product') {
          opt = [...productsByTier(step.questionKey).values()]
            .flat()
            .find((o) => o.value === value);
        } else if (step.type === 'colour') {
          opt = [...coloursByTab(step.questionKey).values()]
            .flat()
            .find((o) => o.value === value);
        } else {
          opt = optionsFor(step.questionKey).find(
            (o) => o.value === value
          );
        }
        ...
        >
          <div className="wizard-card">
            <div className="confirmed">
              ...
              <p>
                Our expert will visit your home. We&rsquo;ll inspect
                the walls, suggest the best solution and give you a
                final quotation.
              </p>
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div className="wizard-card">
              ...
              <div className="pnt-fee-strip">
                <span>
                  {category ? category.visitFeeDisplay : '₹—'}
                </span>
      ...
      >
        ...
        >
          ...
          {step.notSureNote && (
            ...
            >
              Pick the one that matters most — not sure? Our
              expert will help you identify the best walls during
              the home visit.
            </p>
          )}
              ...
              <div className="pnt-option-list">
                {optionsFor(step.questionKey).map((opt) => (
                  <OptionCard
                    ...
                    checked={
                      answers[step.questionKey] === opt.value
                    }
          ...
          {step.type === 'brand' && (
            <>
              <BrandPicker
                options={optionsFor('paint_brand')}
                value={answers.paint_brand}
                onSelect={setAnswer('paint_brand')}
              />
          ...
          >
            ...
            <div className="pnt-modal-body pnt-compare-table">
              {optionsFor(step.questionKey).map((opt) => (
                <div
                  key={opt.value}
                  className="pnt-compare-col"
                >
                  <strong>{opt.label}</strong>

                  {opt.price != null && (
                    <span className="pnt-option-price">
                      From {formatRupees(opt.price)}
                    </span>
                  )}
...
function FlowTopBar({ flow, onBack, plain }) {
  return (
    <div className={plain ? 'pnt-top' : 'wizard-top'}>
      ...
      <h1
        className={
          plain ? 'pnt-top-title' : 'wizard-title'
        }
      >
        {flow?.name}
      </h1>
      ...
      <a
        href={`https://wa.me/${contact.phoneRaw}?text=${encodeURIComponent(
          `Hello Supplybase, I need help booking ${flow?.name}.`
        )}`}
        target="_blank"
        rel="noopener noreferrer"
        className={
          plain ? 'pnt-help' : 'wizard-help'
        }
      >
        Need help?
      </a>
